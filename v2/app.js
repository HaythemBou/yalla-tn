...
function makeMap(canvas, tip, opts){
  ...
  async function ripple(cb){
    ...
    litSet = new Set();
    await cb(0); await wait(2200);
    order.slice(0, 8).forEach(i => litSet.add(i));   await cb(1); await wait(2200);
    order.slice(0, 40).forEach(i => litSet.add(i));  await cb(2); await wait(2200);
    inGov.forEach(i => litSet.add(i));               await cb(3); await wait(2400);
    slots.forEach((_, i) => litSet.add(i));          await cb(4); await wait(2600);
    zoomTarget = .16;                                await cb(5); await wait(4200);
    await cb(6);                                     await wait(2600);
    zoomTarget = 1;                                  await wait(1600);
    litSet = null;
  }
...
function paintSteps(){ paintQuick(); const p = $('#picker'); if (!p) return; const st = $$('.picker .step'); if (st[0]){ st[0].classList.toggle('done', !!currentGov); st[0].classList.toggle('on', !currentGov); } if (st[1]){ st[1].classList.toggle('done', !!currentDeleg); st[1].classList.toggle('on', !!currentGov && !currentDeleg); } if (st[2]) st[2].classList.toggle('on', !!currentGov); p.classList.toggle('has-gov', !!currentGov); p.classList.toggle('has-deleg', !!currentDeleg); }
function paintChosen(){ paintSteps(); const box = $('#chosen'); if (!box) return; box.classList.toggle('on', !!currentGov); set('#chosenName','textContent', placeLabel(lang)); const pk = $('#picker'); if (pk) pk.classList.toggle('has', !!currentGov);
  const dl = $('#delegs'); if (dl){ $$('button', dl).forEach(b => b.remove()); const list = currentGov ? (DELEGS[currentGov] || []) : []; dl.hidden = !list.length;
    list.forEach(d => { const b = document.createElement('button'); b.type = 'button'; b.textContent = delegLabel(currentGov, d[0], lang); b.setAttribute('aria-pressed', String(d[0] === currentDeleg)); b.onclick = () => { pickDeleg(d[0]); const r = $('#rippleBtn'); if (r) r.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth', block: 'center' }); }; dl.appendChild(b); });
    const sk = document.createElement('button'); sk.type = 'button'; sk.className = 'skip'; sk.textContent = t('map.delegSkip'); sk.onclick = () => { const r = $('#rippleBtn'); if (r) r.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth', block: 'center' }); }; dl.appendChild(sk); } }
...
function buildQuick(){ const box = $('#quick'); if (!box) return; $$('button', box).forEach(b => b.remove()); (window.PLACES || []).map(p => p[1]).forEach(ar => { if (!govRow(ar)) return; const b = document.createElement('button'); b.type = 'button'; b.dataset.gov = ar; b.textContent = govLabel(ar); b.onclick = () => { choosePlace({ gov:ar, deleg:'' }); const d = $('#delegs'); if (d) d.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth', block: 'center' }); }; box.appendChild(b); }); paintQuick(); }
function paintQuick(){ $$('#quick button').forEach(b => b.classList.toggle('on', b.dataset.gov === currentGov)); }
...
(function rippleUI(){
  ...
  btn.onclick = async () => { btn.disabled = true; endEl.textContent = ''; document.body.classList.add('ripple');
    ...
    document.body.classList.remove('ripple'); endEl.textContent = t('map.end'); btn.disabled = false; set('#rippleBtn span','textContent', t('map.again')); markStep('place'); };
})();
...
/* the panel next to the phone follows the same conversation, in big type */
function sideSet(kind, text, n){ const box = $('#sideBody'); if (!box) return; const cap = { p: t('day.problem'), no: t('day.without'), x: t('day.with'), take: t('day.part') }[kind] || '';
  box.className = 'sidebody ' + kind; box.innerHTML = '<b>' + esc(cap) + '</b>' + glowYalla(text).replace(/\n/g, '<br>');
  set('#sideNo', 'textContent', String(n || '')); const sc = $('#sideCount'); if (sc) sc.hidden = !n; }
...
async function chatAdvance(){ const sc = dayScript(); const step = sc[chatStep]; if (!step) return;
  ...
  if (step.p){ sceneN++; set('#sceneNo','textContent', String(sceneN)); sideSet('p', step.p.join('\n'), sceneN); let first = true; for (const ln of step.p){ await whenRead(); const pb = bubble('p', ln); if (first){ pb.dataset.cap = t('day.scene') + ' ' + sceneN; first = false; } await sleep(reduced ? 40 : 900); }
    ...
    chipsSet(step.c.map(([label, without, withY]) => [label, '', async () => { bubble('u', label); answers.push(label); await sleep(reduced ? 40 : 600); await whenRead();
      ...
      sideSet('take', (step.t || []).join('\n'), sceneN); await whenRead(); if (step.t){ const tb = bubble('take', step.t.join('\n')); tb.dataset.cap = t('day.part'); }
